...
function forEach(array, callback) {
    ///runs every element in an array through given function
...
function reduce(array, callback, initialValue) {
    // runs function for each with initial value
    var accumulator = initialValue
    if (!accumulator) {
        accumulator = array.shift(0)
    }
    forEach(array, function(v, i, arr) {
        accumulator = callback(accumulator, v, i, arr)
    })
    return accumulator
}
...
// tests
// ---
console.assert(reduce([1, 2, 3, 4], function(a, v) {
    return a * v
}) === 24)
...
function sum() {
    var numArray = [].slice.call(arguments)
    return reduce(numArray, function(v, acc) {
        return v + acc
    })
}
...
var results = customers
    .filter(function(ol) {
        return (ol.first[0].toUpperCase() === "J")
    })
    .map(function(ol) {
        var newobj = {}
        newobj.fullname =(ol.first + ' ' + ol.last)
        return newobj
    })
    .sort(function(a, b) {
